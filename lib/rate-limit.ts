...
export const rateLimitConfigs = {
  auth: {
    requests: isDevelopment ? 1000 : 5, // 5 requests per minute (more lenient in dev)
    interval: 60 * 1000,
  },
  authSignIn: {
    requests: isDevelopment ? 1000 : 10, // 10 sign-in attempts per minute (increased from 3)
    interval: 60 * 1000,
  },
  authSignUp: {
    requests: isDevelopment ? 1000 : 5, // 5 sign-ups per minute (increased from 2)
    interval: 60 * 1000,
  },
  passwordReset: {
    requests: isDevelopment ? 1000 : 5, // 5 password reset requests per minute (increased from 2)
    interval: 60 * 1000,
  },
  admin: {
