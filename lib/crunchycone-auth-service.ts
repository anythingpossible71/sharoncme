...
export function getCrunchyConeAuthService(
  ...
  if (!globalAuthService) {
    ...
    logger.debug("Creating CrunchyCone auth service", {
      isPlatform,
...
export async function checkCrunchyConeAuth(): Promise<CrunchyConeAuthResult> {
  try {
    ...
    if (isPlatform) {
      logger.info("Platform environment detected", {
        platform: process.env.CRUNCHYCONE_PLATFORM,
        hasApiKey: !!process.env.CRUNCHYCONE_API_KEY,
        apiKeyLength: process.env.CRUNCHYCONE_API_KEY?.length,
      });

      // Additional validation for platform environment
      if (!process.env.CRUNCHYCONE_API_KEY) {
        logger.error("Platform environment missing CRUNCHYCONE_API_KEY");
        return {
          success: false,
          source: "api",
      ...
      if (process.env.CRUNCHYCONE_API_KEY.length < 10) {
        logger.warn("Platform CRUNCHYCONE_API_KEY seems too short (less than 10 chars)");
      }
    }

    logger.debug("About to call authService.checkAuthentication()");
    ...
    if (isPlatform && result.success && result.source === "cli") {
      logger.warn(
        "Platform environment got CLI authentication instead of API. CLI may not be available in production."
      );
    }
    ...
    logger.error(
      "Auth check failed with exception",
      {},
...
export async function getCrunchyConeAPIKey(): Promise<string | null> {
  try {
    ...
    logger.error(
      "Failed to get CrunchyCone API key",
      {},
      error instanceof Error ? error : undefined
    );
    return null;
