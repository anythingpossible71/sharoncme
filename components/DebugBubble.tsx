...
export function DebugBubble() {
  ...
  const [isInIframe] = useState(() => {
    ...
    try {
      ...
      if (inIframe) {
        console.log("🔵 [DebugBubble] Iframe identified - hiding bubble:", {
      ...
      console.log("🔵 [DebugBubble] Iframe identified (cross-origin) - hiding bubble:", {
        url: window.location.href,
        pathname: window.location.pathname,
        origin: window.location.origin,
  ...
  const getInitialCorner = (): Corner => {
    if (typeof window !== "undefined") {
      ...
      return savedCorner || "bottom-left";
    }
    return "bottom-left";
  };
  ...
  const getCornerPosition = (corner: Corner): Position => {
    const margin = 16;
