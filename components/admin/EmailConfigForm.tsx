...
export function EmailConfigForm() {
  ...
  >({});
  const [successfulTestProviders, setSuccessfulTestProviders] = useState<Set<string>>(new Set());
  ...
  const checkProviderAvailability = async (provider: EmailProvider) => {
    ...
    try {
      ...
      logger.error(
        `Failed to check availability for ${provider}`,
        {},
  ...
  useEffect(() => {
    async function loadSettings() {
      try {
        const [currentSettings, providersResult] = await Promise.all([
          getCurrentEmailSettings(),
          getConfiguredEmailProviders(),
        ]);
  ...
  const handleProviderChange = async (provider: EmailProvider) => {
    // Clear any existing error/success messages when provider changes
    setMessage(null);
    ...
    // Note: We don't clear successfulTestProviders here because the user might want to see
    // that they've successfully tested a provider even after switching away and back
  };
  ...
  const handleTestConfiguration = async (customTo?: string) => {
    ...
    try {
      ...
      if (result.success) {
        setMessage({ type: "success", text: result.message });
        // Track successful test for providers that have availability checks
        const providersToTrack = ["sendgrid", "resend", "aws-ses"];
        if (providersToTrack.includes(settings.provider)) {
          setSuccessfulTestProviders((prev) => new Set(prev).add(settings.provider));
        }
      } else {
        setMessage({ type: "error", text: result.message });
        // Remove from successful tests if test fails
        setSuccessfulTestProviders((prev) => {
          const newSet = new Set(prev);
          newSet.delete(settings.provider);
          return newSet;
        });
      }
    } catch {
    ...
    <Card>
      <CardContent className="pt-5">
        <form onSubmit={handleSubmit} className="space-y-6 max-w-[600px]">
          ...
          <div className="space-y-4">
            <div className="grid gap-2">
              ...
              <Select value={settings.provider} onValueChange={handleProviderChange}>
                ...
                <SelectContent>
                  <SelectItem value="crunchycone">Crunchycone mail</SelectItem>
                  <SelectItem value="sendgrid">
                    SendGrid{configuredProviders.includes("sendgrid") ? " (Connected)" : ""}
                  </SelectItem>
                  <SelectItem value="resend">
                    Resend{configuredProviders.includes("resend") ? " (Connected)" : ""}
                  </SelectItem>
                  <SelectItem value="aws-ses">
                    Amazon SES{configuredProviders.includes("aws-ses") ? " (Connected)" : ""}
                  </SelectItem>
                  <SelectItem value="smtp">
                    SMTP{configuredProviders.includes("smtp") ? " (Connected)" : ""}
                  </SelectItem>
                  <SelectItem value="mailgun">
                    Mailgun{configuredProviders.includes("mailgun") ? " (Connected)" : ""}
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
          ...
          {(settings.provider === "sendgrid" ||
            ...
            <>
              {providerAvailability[settings.provider]?.checking && (
                <Alert>
                  <AlertDescription>Checking {settings.provider} availability...</AlertDescription>
                </Alert>
              )}
              ...
              {providerAvailability[settings.provider] &&
                !providerAvailability[settings.provider].checking &&
                providerAvailability[settings.provider].available &&
                successfulTestProviders.has(settings.provider) && (
                  <Alert>
                    <AlertDescription>
          ...
          {/* CrunchyCone Auth Status Alert */}
          {settings.provider === "crunchycone" && crunchyConeAuthStatus !== null && (
                ...
                {crunchyConeAuthStatus.authenticated ? (
                  ...
                  <>
                    ...
                    <span className="block text-xs mt-1">
                      Run:{" "}
                      <code className="bg-muted px-1 rounded">npx crunchycone-cli auth login</code>
                    </span>
                  </>
                )}
              </AlertDescription>
          ...
          {/* CrunchyCone Info Alert */}
          {settings.provider === "crunchycone" && (
            <Alert>
              <AlertDescription>
