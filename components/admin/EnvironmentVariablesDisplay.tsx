...
import { Eye, EyeOff, Copy, Check, Edit2, Trash2, Loader2, RefreshCw, Cloud } from "lucide-react";
...
export interface EnvironmentVariablesDisplayRef {
  openAddDialog: () => void;
}

export const EnvironmentVariablesDisplay = forwardRef<EnvironmentVariablesDisplayRef>(
  (props, ref) => {
    ...
    const [_crunchyConeAuth, setCrunchyConeAuth] = useState<{
      isAuthenticated: boolean;
      source: string;
    }>({ isAuthenticated: false, source: "unknown" });
    const [_crunchyConeStats, setCrunchyConeStats] = useState<{
      envCount: number;
      secretCount: number;
    ...
    // Expose openAddDialog to parent component
    ...
    const _openPullDialog = (variableKey: string, localValue: string, remoteValue: string) => {
      setSyncDialog({
        isOpen: true,
        variableKey,
        direction: "pull",
    ...
    const _openPushDialog = (variableKey: string, localValue: string, remoteValue: string) => {
    ...
    const handleSync = async () => {
      try {
        ...
        logger.error(
          "Error syncing environment variable",
          {},
          err instanceof Error ? err : undefined
        );
        setSyncLoading(false);
        ...
        <CardContent className="p-6 pt-6">
          <div className="w-full">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-auto whitespace-nowrap">Variable Name</TableHead>
                  {platform.isPlatformEnvironment ? (
                    <TableHead className="w-auto">Platform Value</TableHead>
                  ) : (
                  ...
                  <TableRow key={envVar.key}>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        {isDeploymentSpecific(envVar.key) && envVar.key !== "AUTH_URL" && (
                          <TooltipProvider>
                            <Tooltip
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              {!envVar.isRemoteSecret ? (
                                <Button
                                  ...
                                  onClick={() =>
                                    openEditDialog(envVar.key, envVar.remoteValue || "")
                                  }
                                  title="Edit variable"
                                >
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  value={editDialog.editValue}
                  onChange={(e) =>
                    setEditDialog((prev) => ({ ...prev, editValue: e.target.value }))
                  }
                  className="font-mono"
...
EnvironmentVariablesDisplay.displayName = "EnvironmentVariablesDisplay";
