...
export interface UserManagementPanelRef {
  openCreateDialog: () => void;
}

export const UserManagementPanel = forwardRef<UserManagementPanelRef, UserManagementPanelProps>(
  ({ initialUsers, totalCount, currentPage, itemsPerPage, currentUserId, availableRoles }, ref) => {
    ...
    // Expose openCreateDialog to parent component
    ...
    async function handleSearch(e: React.FormEvent) {
      ...
      if (searchTerm) {
        router.push(`/admin/authentication?tab=users&search=${encodeURIComponent(searchTerm)}`);
      } else {
        router.push(`/admin/authentication?tab=users`);
      }
    }
    ...
    async function handleCreateUser(e: React.FormEvent) {
      ...
      try {
        ...
        logger.info("Sending create user request", { requestData });
        ...
        logger.error("Create user error caught", {}, error instanceof Error ? error : undefined);
                      ...
                      <div className="flex items-center justify-center h-8 w-8 rounded-full bg-muted overflow-hidden">
                        {user.image ? (
                          <Image
                            ...
                            onError={(e) => {
                              logger.info("Image failed to load", { userImage: user.image });
            ...
            <div className="flex items-center justify-center gap-2">
              <Button
                ...
                onClick={() => {
                  ...
                  const url = search
                    ? `/admin/authentication?tab=users&page=${currentPage - 1}&search=${encodeURIComponent(search)}`
                    : `/admin/authentication?tab=users&page=${currentPage - 1}`;
                  router.push(url);
                }}
                  ...
                  <div className="grid gap-2">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-xs break-all">
                        {selectedUser.image || "Not provided"}
                      </span>
                    </div>
                    <div className="flex justify-between">
