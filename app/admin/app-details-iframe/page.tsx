...
export default function PublishSettingsPage() {
  ...
  const handlePublish = async () => {
    // If there are changes, save first, then redirect
    if (hasDetailsChanges) {
      const saveSuccess = await handleSaveDetails();
      // Only redirect if save was successful
      if (saveSuccess) {
        // Redirect the iframe (not the parent window)
        window.location.href = "/admin/dev-build-progress-iframe";
      }
      // If save failed, don't redirect (error already shown via toast)
    } else {
      // No changes, just redirect to publish progress
      window.location.href = "/admin/dev-build-progress-iframe";
    }
  };
  ...
  const detailsButtonLabel = saveSuccess
    ...
    : isSavingDetails
      ...
      : hasDetailsChanges
        ? "Save change & publish"
        : "Publish";
  const isDetailsButtonDisabled = isLoading || isSavingDetails;

  return (
    <div className="min-h-screen bg-background p-0 pb-20">
      <div className="space-y-6 p-0">
        <div className="space-y-6">
          {/* App Name */}
          <div className="pt-0 pb-0 px-6 mt-3 mb-0">
      ...
      {/* Fixed Save Button Bar - Fixed to bottom */}
      <div className="fixed bottom-0 left-0 right-0 flex justify-start py-[10px] px-6 mt-0 bg-background border-t shadow-lg z-50">
        <Button
