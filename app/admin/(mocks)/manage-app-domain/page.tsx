...
export default function ManageAppDomainPage() {
  ...
  const handleSaveChanges = () => {
    if (!isTaken && subdomain.trim() && subdomain !== savedSubdomain) {
      setSavedSubdomain(subdomain);
      // Here you could also save to backend/database if needed
    }
  };
  ...
  const handlePublish = async () => {
    if (!isTaken && subdomain.trim()) {
      // If there are changes, save first, then navigate
      if (hasDetailsChanges) {
        const saveSuccess = await handleSaveDetails();
        // Only navigate if save was successful
        if (saveSuccess) {
          router.push(`/admin/dev-build-progress?subdomain=${subdomain}`);
        }
        // If save failed, don't navigate (error already shown via toast)
      } else {
        // No changes, just navigate to progress page
      router.push(`/admin/dev-build-progress?subdomain=${subdomain}`);
      }
    }
  };

  const publishButtonText = saveSuccess
    ? "Changes saved"
    : isSavingDetails
      ? "Saving..."
      : hasDetailsChanges
        ...
        : hasPublished
          ? "Publish changes"
          : "Publish app";
  const isPublishButtonDisabled = !isAvailable || isTaken || !subdomain.trim() || isLoadingDetails || isSavingDetails;

  return (
    <div className="w-full max-w-lg min-h-screen bg-background">
      {/* Main Content */}
      <div style={{ padding: '20px', paddingBottom: '100px' }}>
        <div className="space-y-6">
          ...
          <SubdomainInput
            ...
            label="Manage your app domain"
            placeholder="myproject"
            takenDomains={takenDomains}
              originalValue={savedSubdomain}
          ...
          <button
            type="button"
            onClick={() => setIsAppDetailsExpanded(!isAppDetailsExpanded)}
          ...
          {isAppDetailsExpanded && (
            <div className="space-y-3 mt-3">
              ...
              {appDetailsMode === 'minimal' ? (
                <span className="text-sm font-medium mb-1.5 block">App Name</span>
              ) : (
                <h6 className="mb-1.5">App Name</h6>
              )}
              <Input
                id="app-name"
                  type="text"
