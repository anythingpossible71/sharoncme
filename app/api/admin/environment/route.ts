...
export async function GET(_request: NextRequest) {
  try {
    ...
    logger.debug("Environment API GET Request - Authentication passed using requireRole");
    ...
    try {
      if (isInPlatformMode) {
        ...
        logger.debug("Platform mode - about to fetch environment variables from CrunchyCone API");
        logger.debug("Environment service provider info", { providerInfo });

        // Get environment variables from platform
        logger.debug("Calling envService.listEnvVars()");
        const envVars = await envService.listEnvVars();
        logger.debug("Successfully fetched envVars", { count: Object.keys(envVars).length });
        ...
        if (hasCrunchyConeConfig) {
          ...
          logger.debug("No crunchycone.toml found - loading only local environment variables");
    ...
    if (!isInPlatformMode) {
      ...
      if (!hasCrunchyConeConfig) {
        logger.debug("No crunchycone.toml found - project not available in CrunchyCone");
        crunchyConeAuth = {
        ...
        try {
          ...
          logger.warn("Failed to determine CrunchyCone auth status", {
