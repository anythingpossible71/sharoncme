...
async function checkCrunchyConeAuth(): Promise<boolean> {
  try {
    ...
    logger.error(
      "Failed to check CrunchyCone authentication",
      {},
      error instanceof Error ? error : undefined
    );
    return false;
...
async function getEnvironmentData() {
  ...
  try {
    ...
    logger.error("Error reading .env file", {}, error instanceof Error ? error : undefined);
  }
  ...
  try {
    ...
    logger.error("Error fetching CrunchyCone data", {}, error instanceof Error ? error : undefined);
  }
...
async function fetchCrunchyConeVariables(): Promise<{
  ...
  try {
    ...
    try {
      ...
      logger.error(
        "Failed to fetch CrunchyCone env vars",
        {},
        error instanceof Error ? error : undefined
      );
    }
    ...
    try {
      ...
      logger.error(
        "Failed to fetch CrunchyCone secrets",
        {},
        error instanceof Error ? error : undefined
      );
    }
    ...
    logger.error(
      "Failed to fetch CrunchyCone data",
      {},
      error instanceof Error ? error : undefined
    );
    return { envVars: {}, secrets: {} };
...
async function syncVariable(
  ...
  try {
    ...
    logger.error("Error syncing variable", { key }, error instanceof Error ? error : undefined);
    throw new Error(
  ...
  try {
    if (isSecret) {
      ...
      try {
        ...
        logger.debug("Env var not found or already deleted", { key });
      }
      ...
      try {
        ...
        logger.debug("Secret not found or already deleted", { key });
      }
