  ...
  try {
    ...
    try {
      ...
      if (uploadResult.success) {
        ...
        logger.info("Database upload triggered successfully", {
          userId: user.id,
          action: "setup-admin",
        });
      } else {
        // Log but don't fail the admin setup
        logger.info("Database upload not triggered", {
      ...
      // Log but don't fail the admin setup
      logger.error(
        "Failed to trigger database upload",
        { userId: user.id, action: "setup-admin" },
