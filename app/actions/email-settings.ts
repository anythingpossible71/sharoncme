...
async function sendTestEmail(
  ...
  try {
    ...
    logger.info(`Provider: ${settings.provider.toUpperCase()}`);
    logger.info(
      `From: ${settings.fromAddress} ${settings.fromDisplayName ? `(${settings.fromDisplayName})` : ""}`
    );
    logger.info(`To: ${options.to}`);
    logger.info(`Subject: ${options.subject}`);
    logger.info(`✅ Email sent successfully using ${settings.provider.toUpperCase()}`);
    logger.info("===============================");
  } catch (error) {
    ...
    logger.info("=== EMAIL TEST CONFIGURATION (Fallback) ===");
    logger.info(`Provider: ${settings.provider.toUpperCase()}`);
    logger.info(
      `From: ${settings.fromAddress} ${settings.fromDisplayName ? `(${settings.fromDisplayName})` : ""}`
    );
    ...
    switch (settings.provider) {
      case "smtp":
        logger.info(`SMTP Host: ${settings.smtpHost || "[NOT SET]"}`);
        logger.info(`SMTP Port: ${settings.smtpPort || "[NOT SET]"}`);
        logger.info(`SMTP User: ${settings.smtpUser || "[NOT SET]"}`);
        logger.info(`SMTP Password: ${settings.smtpPassword ? "[SET]" : "[NOT SET]"}`);
        break;
      case "sendgrid":
        logger.info(`SendGrid API Key: ${settings.sendgridApiKey ? "[SET]" : "[NOT SET]"}`);
        break;
      case "resend":
        logger.info(`Resend API Key: ${settings.resendApiKey ? "[SET]" : "[NOT SET]"}`);
        break;
      case "aws-ses":
        logger.info(`AWS Region: ${settings.awsRegion || "[NOT SET]"}`);
        logger.info(`AWS Access Key ID: ${settings.awsAccessKeyId ? "[SET]" : "[NOT SET]"}`);
        logger.info(
          `AWS Secret Access Key: ${settings.awsSecretAccessKey ? "[SET]" : "[NOT SET]"}`
        );
        break;
      case "mailgun":
        logger.info(`Mailgun API Key: ${settings.mailgunApiKey ? "[SET]" : "[NOT SET]"}`);
        logger.info(`Mailgun Domain: ${settings.mailgunDomain || "[NOT SET]"}`);
        break;
      case "crunchycone":
        logger.info("CrunchyCone: Uses CLI authentication");
        break;
    }

    logger.info("--- Email Content ---");
    logger.info(options.text || options.html);
    logger.info("==========================================");
    logger.warn(`⚠️  Email configuration validated for ${settings.provider.toUpperCase()}`);
    logger.info("Error occurred during actual email sending - check configuration");
...
export type EmailProvider = "crunchycone" | "sendgrid" | "resend" | "aws-ses" | "smtp" | "mailgun";
...
export async function updateEmailSettings(formDataOrSettings: FormData | EmailSettings) {
  ...
  try {
    ...
    logger.error("Failed to update email settings", {}, error instanceof Error ? error : undefined);
    return {
      success: false,
...
export async function getCurrentEmailSettings(): Promise<EmailSettings> {
  ...
  return {
    provider: (envVars.CRUNCHYCONE_EMAIL_PROVIDER as EmailProvider) || "crunchycone",
...
Sent at: ${new Date().toISOString()}
      ...
      try {
        ...
        logger.error("Email test error", {}, error instanceof Error ? error : undefined);
        return {
          success: false,
      ...
      logger.error(
        "Email configuration test error",
        {},
    ...
    logger.error("Email configuration test error", {}, error instanceof Error ? error : undefined);
    return {
      success: false,
      message: "Failed to test email configuration",
    };
  ...
  try {
    ...
    switch (provider) {
      ...
      case "aws-ses":
        ...
        break;
      default:
        // For other providers (smtp, mailgun, crunchycone), assume they're available
        available = true;
    }
...
export async function checkCrunchyConeAuth() {
  ...
  try {
    ...
    if (stderr) {
      logger.error("CrunchyCone CLI stderr", { stderr });
    }
    ...
    logger.error(
      "Failed to check CrunchyCone auth",
      {},
      error instanceof Error ? error : undefined
    );
    return {
...
/**
 * Get list of configured email provider IDs
 */
export async function getConfiguredEmailProviders(): Promise<{
  success: boolean;
  providerIds?: string[];
  error?: string;
}> {
  try {
    ...
    // Filter to email services only and check if they're connected
    const configuredEmailIds = services
      .filter((service) => service.category === "email" && isServiceConnected(service))
      .map((service) => service.id);

    return { success: true, providerIds: configuredEmailIds };
  } catch (error) {
