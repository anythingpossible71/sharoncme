...
export async function getStorageSettings(): Promise<{
  ...
  try {
    ...
    const settings: StorageSettings = {
      ...
      wasabiAccessKey: envVars.CRUNCHYCONE_WASABI_ACCESS_KEY,
      wasabiSecretKey: envVars.CRUNCHYCONE_WASABI_SECRET_KEY,
      wasabiRegion: envVars.CRUNCHYCONE_WASABI_REGION,
      wasabiBucket: envVars.CRUNCHYCONE_WASABI_BUCKET,
      r2AccessKeyId: envVars.CRUNCHYCONE_R2_ACCESS_KEY_ID,
      r2SecretAccessKey: envVars.CRUNCHYCONE_R2_SECRET_ACCESS_KEY,
      r2AccountId: envVars.CRUNCHYCONE_R2_ACCOUNT_ID,
      r2Bucket: envVars.CRUNCHYCONE_R2_BUCKET,
      r2PublicDomain: envVars.CRUNCHYCONE_R2_PUBLIC_DOMAIN,
      backblazeKeyId: envVars.CRUNCHYCONE_BACKBLAZE_KEY_ID,
      backblazeApplicationKey: envVars.CRUNCHYCONE_BACKBLAZE_APPLICATION_KEY,
      backblazeRegion: envVars.CRUNCHYCONE_BACKBLAZE_REGION,
      backblazeBucket: envVars.CRUNCHYCONE_BACKBLAZE_BUCKET,
...
function isServiceConnected(service: ServiceConfig): boolean {
...
/**
 * Get list of configured storage provider IDs
 */
export async function getConfiguredStorageProviders(): Promise<{
  success: boolean;
  providerIds?: string[];
  error?: string;
}> {
  try {
    ...
    // Filter to storage services only and check if they're connected
    const configuredStorageIds = services
      .filter((service) => service.category === "storage" && isServiceConnected(service))
      .map((service) => service.id);

    return { success: true, providerIds: configuredStorageIds };
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : "Unknown error";
    return { success: false, error: `Failed to get configured providers: ${errorMessage}` };
  }
}
...
export async function updateStorageSettings(
  ...
  try {
    ...
    const updates: Record<string, string | undefined> = {
      ...
      CRUNCHYCONE_WASABI_ACCESS_KEY: settings.wasabiAccessKey,
      CRUNCHYCONE_WASABI_SECRET_KEY: settings.wasabiSecretKey,
      CRUNCHYCONE_WASABI_REGION: settings.wasabiRegion,
      CRUNCHYCONE_WASABI_BUCKET: settings.wasabiBucket,
      CRUNCHYCONE_R2_ACCESS_KEY_ID: settings.r2AccessKeyId,
      CRUNCHYCONE_R2_SECRET_ACCESS_KEY: settings.r2SecretAccessKey,
      CRUNCHYCONE_R2_ACCOUNT_ID: settings.r2AccountId,
      CRUNCHYCONE_R2_BUCKET: settings.r2Bucket,
      CRUNCHYCONE_R2_PUBLIC_DOMAIN: settings.r2PublicDomain,
      CRUNCHYCONE_BACKBLAZE_KEY_ID: settings.backblazeKeyId,
      CRUNCHYCONE_BACKBLAZE_APPLICATION_KEY: settings.backblazeApplicationKey,
      CRUNCHYCONE_BACKBLAZE_REGION: settings.backblazeRegion,
      CRUNCHYCONE_BACKBLAZE_BUCKET: settings.backblazeBucket,
    };
    ...
    logger.error("Error updating storage settings", {}, error instanceof Error ? error : undefined);
    const errorMessage = error instanceof Error ? error.message : "Unknown error";
  ...
  try {
    ...
    switch (settings.provider) {
      ...
      case "gcp":
        ...
        break;
      case "wasabi":
        if (settings.wasabiAccessKey)
          testEnvVars.CRUNCHYCONE_WASABI_ACCESS_KEY = settings.wasabiAccessKey;
        if (settings.wasabiSecretKey)
          testEnvVars.CRUNCHYCONE_WASABI_SECRET_KEY = settings.wasabiSecretKey;
        if (settings.wasabiRegion) testEnvVars.CRUNCHYCONE_WASABI_REGION = settings.wasabiRegion;
        if (settings.wasabiBucket) testEnvVars.CRUNCHYCONE_WASABI_BUCKET = settings.wasabiBucket;
        break;
      case "r2":
        if (settings.r2AccessKeyId)
          testEnvVars.CRUNCHYCONE_R2_ACCESS_KEY_ID = settings.r2AccessKeyId;
        if (settings.r2SecretAccessKey)
          testEnvVars.CRUNCHYCONE_R2_SECRET_ACCESS_KEY = settings.r2SecretAccessKey;
        if (settings.r2AccountId) testEnvVars.CRUNCHYCONE_R2_ACCOUNT_ID = settings.r2AccountId;
        if (settings.r2Bucket) testEnvVars.CRUNCHYCONE_R2_BUCKET = settings.r2Bucket;
        if (settings.r2PublicDomain)
          testEnvVars.CRUNCHYCONE_R2_PUBLIC_DOMAIN = settings.r2PublicDomain;
        break;
      case "backblaze":
        if (settings.backblazeKeyId)
          testEnvVars.CRUNCHYCONE_BACKBLAZE_KEY_ID = settings.backblazeKeyId;
        if (settings.backblazeApplicationKey)
          testEnvVars.CRUNCHYCONE_BACKBLAZE_APPLICATION_KEY = settings.backblazeApplicationKey;
        if (settings.backblazeRegion)
          testEnvVars.CRUNCHYCONE_BACKBLAZE_REGION = settings.backblazeRegion;
        if (settings.backblazeBucket)
          testEnvVars.CRUNCHYCONE_BACKBLAZE_BUCKET = settings.backblazeBucket;
        break;
      case "crunchycone":
