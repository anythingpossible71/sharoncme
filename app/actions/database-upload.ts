...
export async function triggerDatabaseUpload(): Promise<{ success: boolean; message: string }> {
  try {
    ...
    const child = spawn("npx", ["--yes", "crunchycone-cli", "database", "upload"], {
      ...
      shell: true, // Use shell to ensure npx is found in PATH
    });
    ...
    child.on("close", async (code) => {
      ...
      try {
        ...
        logger.error("Failed to update status file after process close", {
    ...
    child.on("error", async (error) => {
      ...
      try {
        ...
        logger.error("Failed to update status file after spawn error", {
    ...
    logger.error("Error triggering database upload", {
      error: error instanceof Error ? error.message : String(error),
    });
    ...
    try {
      ...
      logger.error("Failed to update status file in catch block", {
        error: writeError instanceof Error ? writeError.message : String(writeError),
      });
    }
